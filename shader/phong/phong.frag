...
vec3 calculatePointLight(PointLight light, vec3 normal, vec3 viewDir, vec3 fragPos) {
	vec3 ambient = 0.2 * light.color * vec3(texture(material.diffuse1, texCoord)) * factor.ambient;

	vec3 lightDir = normalize(light.pos - fragPos);
	float diff = max(dot(normal, lightDir), 0);
	vec3 diffuse = 0.8 * light.color * diff * vec3(texture(material.diffuse1, texCoord)) * factor.diffuse;
	
	float spec;
	if(blinn) {
		vec3 halfVec = normalize(lightDir + viewDir);
		spec = pow(max(dot(halfVec, normal), 0), material.shininess);
	}
	else {
		vec3 reflectDir = reflect(-lightDir, normal);
	...
	vec3 specular = 1.0 * light.color * spec * vec3(texture(material.specular1, texCoord)) * factor.specular;
...
void main() {
	...
	vec3 result = vec3(0);
	result += calculatePointLight(light, normal, viewDir, fragPos);
