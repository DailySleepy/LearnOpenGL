...
const int clusterCount = tileCountX * tileCountY * depthSliceCount; // 23104

float R_max = 4.0;
bool freezeView = false;
bool enablePlaneIntersection = true;
bool enableAABBIntersection = true;
bool enableLogDepthSlice = true;
bool enableClusterActivation = true;
...
int main()
{
	...
	Shader sClusterActive(
		getPath("shader/deferred/clustered/clusterActive.comp"), ShaderType::Compute);
	Shader sClusterLight(
		getPath("shader/deferred/clustered/clusterLight_detectActive.comp"), ShaderType::Compute);
	...
	int clusterBufferSize = clusterCount * sizeof(int);
	StorageBuffer ssbo_clusterActive(clusterBufferSize);
	vector<int> initActive(clusterBufferSize, 0u);
	...
	Shader::UniformMap u_sClusterActive = {
	...
	Shader::UniformMap u_sClusterLight = {
		...
		{"enableLogDepthSlice", enableLogDepthSlice},
		{"enableClusterActivation", enableClusterActivation},
	};
	...
	while (!window.shouldClose())
	{
		...
		ssbo_clusterActive.setSubData(0, clusterCount, initActive.data());
		...
		// compute active cluster
		if (enableClusterActivation)
		{
			sClusterActive.bind();
			sClusteredDeferred.bindSSBO(ssbo_clusterActive.getSSBO(), 0);
			u_sClusterActive["enableLogDepthSlice"] = enableLogDepthSlice;
			sClusterActive.set(u_sClusterActive);
		...
		// compute cluster light list
		{
			...
			sClusterLight.bindSSBO(ssbo_clusterActive.getSSBO(), 2);
			...
			u_sClusterLight["enableClusterActivation"] = enableClusterActivation;
	...
	renderingOver:
		...
		ImGui::Checkbox("Enable Cluster Activation", &enableClusterActivation);
