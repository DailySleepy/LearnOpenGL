...
const int maxLightsPerTile = 512;
...
int main()
{
	...
	Shader sDepthRange(
		getPath("shader/deferred/tiled/depthProjRange.comp"), ShaderType::Compute);
	Shader sTransformLight(
		getPath("shader/deferred/transformLight.comp"), ShaderType::Compute);
	Shader sTileLight(
		getPath("shader/deferred/tiled/tileLight_zProjRange.comp"), ShaderType::Compute);
	...
	StorageBuffer ssbo_depthProjRangeInt(tileCount * sizeof(ivec2));
	StorageBuffer ssbo_debug(tileCount * sizeof(vec2));
	vector<ivec2> depthInit(tileCount, ivec2(floatBitsToInt(1.0f), floatBitsToInt(-1.0f)));
	...
	while (!window.shouldClose())
	{
		...
		ssbo_depthProjRangeInt.setSubData(0, tileCount * sizeof(ivec2), depthInit.data());
		...
		// tile lights - compute shader
		{
			...
			sDepthRange.bindSSBO(ssbo_depthProjRangeInt.getSSBO(), 0);
			sDepthRange.setHandle("depthTexture", G_Buffer.getDepthTextureHandle());
			...
			sTileLight.bindSSBO(ssbo_depthProjRangeInt.getSSBO(), 2);
		...
		// render tiles
		{
			...
			sTiledDeferred.setHandle("gPosition", G_Buffer.getColorTextureHandle(0));
			sTiledDeferred.setHandle("gNormal", G_Buffer.getColorTextureHandle(1));
			sTiledDeferred.setHandle("gAlbedoSpec", G_Buffer.getColorTextureHandle(2));
