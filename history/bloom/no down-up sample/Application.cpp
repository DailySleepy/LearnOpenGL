...
int main()
{
	...
	vector<vec3> lightColor = {
		10.1f * vec3(1.0f, 1.0f, 0.9f),
		10.1f * vec3(0.1f, 0.0f, 0.0f),
		10.1f * vec3(0.0f, 0.0f, 0.2f),
		10.1f * vec3(0.0f, 0.1f, 0.0f)
	};
	...
	FrameBuffer blurFBO[2]{
		FrameBuffer(WIDTH, HEIGHT, FrameBufferType::COLOR_FLOAT),
		FrameBuffer(WIDTH, HEIGHT, FrameBufferType::COLOR_FLOAT) };
	...
	while (!window.shouldClose())
	{
		...
		if (useBloom)
		{
			...
			for (int i = 0; i < 2 * times; i++)
			{
				...
				renderer.clearAllBit();
				if (isFirstFrame) sceneFBO.bindColorTex(0, 1);
				else blurFBO[!isHorizontal].bindColorTex();
		...
		else
		{
			...
			renderer.clearAllBit();
			sceneFBO.bindColorTex(0, 0);
			//sceneFBO.bindColorTex(0, 1);
			//blurFBO[1].bindColorTex(0, 0);
			sShowOnScreen.bind();
