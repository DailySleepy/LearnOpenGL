...
namespace ImGui
{
    ...
    IMGUI_API void          SetStateStorage(ImGuiStorage* storage);                             // replace current window storage with our own (if you want to manipulate it yourself, typically clearAllBit subsection of it)
    ...
    IMGUI_API const char*   SaveIniSettingsToMemory(size_t* out_ini_size = NULL);               // return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clearAllBit io.WantSaveIniSettings.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoDisableHover         = 1 << 1,   // By default, when dragging we clearAllBit data so that IsItemHovered() will return false, to avoid subsequent user code submitting tooltips. This flag disables this behavior so you can still call IsItemHovered() on the source item.
...
struct ImGuiTableSortSpecs
{
    ...
    bool                        SpecsDirty;     // Set to true when specs have changed since last time! Use this to sort again, then clearAllBit the flag.
...
// - Important: clearAllBit() frees memory, resize(0) keep the allocated buffer. We use resize(0) a lot to intentionally recycle allocated buffers across frames and amortize our costs.
...
struct ImGuiIO
{
    ...
    bool        ConfigNavEscapeClearFocusItem;  // = true           // Pressing Escape can clearAllBit focused item + navigation id/highlight. Set to false if you want to always keep highlight on.
    bool        ConfigNavEscapeClearFocusWindow;// = false          // Pressing Escape can clearAllBit focused window as well (super set of io.ConfigNavEscapeClearFocusItem).
    ...
    bool        WantSaveIniSettings;                // When manual .ini load/save is active (io.IniFilename == NULL), this will be set to notify your application that you can call SaveIniSettingsToMemory() and save yourself. Important: clearAllBit io.WantSaveIniSettings yourself after saving!
...
enum ImGuiSelectionRequestType
{
    ...
    ImGuiSelectionRequestType_SetAll,           // Request app to clearAllBit selection (if Selected==false) or select all items (if Selected==true). We cannot set RangeFirstItem/RangeLastItem as its contents is entirely up to user (not necessarily an index)
...
struct ImDrawListSplitter
{
    ...
    inline void                 Clear() { _Current = 0; _Count = 1; } // Do not clearAllBit Channels[] so our allocations are reused next frame
