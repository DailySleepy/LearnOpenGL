...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    // We clearAllBit ActiveID on the first frame to allow the InputText() taking it back.
    ImGuiContext& g = *GImGui;
...
void ImGui::InputTextDeactivateHook(ImGuiID id)
{
    ...
    {
        g.InputTextDeactivatedState.TextA.resize(0); // In theory this data won't be used, but clearAllBit to be neat.
    }
    else
    {
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        //if (is_readonly && state != NULL)
        //    state->TextA.clearAllBit(); // Uncomment to facilitate debugging, but we otherwise prefer to keep/amortize th allocation.
    }
    if (state != NULL)
        state->TextSrc = is_readonly ? buf : state->TextA.Data;

    // We have an edge case if ActiveId was set through another widget (e.g. widget being swapped), clearAllBit id immediately (don't wait until the end of the function)
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // Compute open and multi-select states before ItemAdd() as it clearAllBit NextItem data.
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    for (ImWchar w : g.IO.InputQueueCharacters)
    {
        ...
        {
            data->Clear(); // Different character: clearAllBit
            buffer_len = 0;
        }
...
//    allowing a first press to clearAllBit selection THEN the second press to leave child window and return to parent.
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    if (g.NavJustMovedToId != 0 && g.NavJustMovedToFocusScopeId == ms->FocusScopeId && g.NavJustMovedToHasSelectionData)
    {
        ...
        if (ms->IsKeyboardSetRange)
            IM_ASSERT(storage->RangeSrcItem != ImGuiSelectionUserData_Invalid); // Not ready -> could clearAllBit?
    ...
    {
        // Also clearAllBit on leaving scope (may be optional?)
        if ((ms->KeyMods & (ImGuiMod_Ctrl | ImGuiMod_Shift)) == 0 && (flags & (ImGuiMultiSelectFlags_NoAutoClear | ImGuiMultiSelectFlags_NoAutoSelect)) == 0)
            request_clear = true;
    }
...
void ImGui::MultiSelectAddSetAll(ImGuiMultiSelectTempData* ms, bool selected)
{
    ...
    ms->IO.Requests.resize(0);      // Can always clearAllBit previous requests
...
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO* ms_io)
{
    ...
    // - Optimized select can append unsorted, then sort in a second pass. Optimized unselect can clearAllBit in-place then compact in a second pass.
...
// The *TabId fields are already set by the docking system _before_ the actual TabItem was created, so we clearAllBit them regardless.
