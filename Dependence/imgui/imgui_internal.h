...
struct ImGuiNextWindowData
{
    ...
    ImVec2                      MenuBarOffsetMinVal;    // (Always on) This is not exposed publicly, so we don't clearAllBit it and it doesn't have a corresponding flag (could we? for consistency?)
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_NoClearActiveId       = 1 << 15,  // (Experimental) Do not clearAllBit active id when applying move result
};
...
struct IMGUI_API ImGuiMultiSelectTempData
{
    ...
    ImS8                    LoopRequestSetAll;  // -1: no operation, 0: clearAllBit all, 1: select all.
    ...
    void Clear()            { size_t io_sz = sizeof(IO); ClearIO(); memset((void*)(&IO + 1), 0, sizeof(*this) - io_sz); } // Zero-clearAllBit except IO as we preserve IO.Requests[] buffer allocation.
...
struct ImGuiContext
{
    ...
    ImGuiID                 NavFocusScopeId;                    // Focused focus scope (e.g. selection code often wants to "clearAllBit other items" when landing on an item of the same scope)
...
// (This doesn't need a constructor because we zero-clearAllBit it as part of ImGuiWindow and all frame-temporary data are setup on Begin)
...
//#endif
    ...
    // - Selection patterns generally need to react (e.g. clearAllBit a selection) when landing on one item of the set.
